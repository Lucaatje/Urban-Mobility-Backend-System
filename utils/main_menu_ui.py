...
def print_main_menu(logged_in_user, prompt):
    ...
    if logged_in_user.role == UserRole.SUPER_ADMIN:
        ...
        print(f'Manage log information: {COMMENT}- Press: 5{RESET}')
        print(f'{BUTTON}[LOG OUT]{RESET} {COMMENT}Press: 6{RESET}')
...
def main_menu_ui(logged_in_user, prompt):
    while(True):
        ...
        if (logged_in_user.role == UserRole.SUPER_ADMIN or logged_in_user.role == UserRole.SYSTEM_ADMIN) and index in ['1', '2', '3', '4', '5', '6']:
            return index
